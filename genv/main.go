...
func setup() {
	...
	_, status, err := qdrant.DoRequest(context.TODO(), *url, "", http.MethodPut, collectionConfig)

	fmt.Println(http.StatusOK, status)
}
func main() {
	...
	store, errNs := qdrant.New(
		qdrant.WithURL(*url),
		qdrant.WithCollectionName("ok"),
